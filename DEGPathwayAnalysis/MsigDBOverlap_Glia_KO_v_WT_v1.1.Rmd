...
res_dir = "MSigDBOverlap/"

# All clusters compared is on Sheet 1
infile = "Neurons_GeneExpression_ConditionsCompared.xlsx"
...
deg_tbl = read.xlsx(paste0(data_dir, infile), sheetIndex = 1) %>% # All clusters compared is on Sheet 1
  # Fix col names
...
datatable(deg_tbl %>%
            ...
            mutate(
              ...
              across(c('avg_logFC', 'avg_fold', 'avg_log2FC'), \(x) format(x, digits = 4, scientific = FALSE))),
                   caption = htmltools::tags$caption(style = 'caption-side: top; text-align: center; color:black;  font-size:200% ;', paste0('DEGs for all clusters'))
            )
...
write.table(genes_up %>% pull(gene),
            file = paste0(res_dir, "MSigDBOverlap/InputLists/", proj, "_",
...
MSigDBOverlap_genes_up = read_tsv(paste0(res_dir, "MSigDBOverlap/Results/",
...
dotplot = ggplot(MSigDBOverlap_genes_up %>%
  ...
  # ylab("Pathway") +
  ggtitle("HIGHER IN TyrBap1") + # "MSigDB Overlap of Glia DEGs up in KO (all clusters): Top 10 Gene Sets"
...
pdf(file = paste0(res_dir, "MSigDBOverlap/Figures/", analysis_name, "_", proj, "_",
                  "AllClusters", "_", "GenesUp", "_", version, ".1.pdf"), height = 10, width = 10)
print(dotplot)
dev.off()

png(file = paste0(res_dir, "MSigDBOverlap/Figures/", analysis_name, "_", proj, "_",
...
MSigDBOverlap_genes_down = read_tsv(paste0(res_dir, "MSigDBOverlap/Results/",
...
dotplot = ggplot(MSigDBOverlap_genes_down %>%
  ...
  xlab("Gene ratio") + # (# Genes in Overlap / # Genes in Gene Set)
  ylab("Pathway") +
  ggtitle("HIGHER IN WT") + # "MSigDB Overlap of Glia DEGs down in KO (all clusters): Top 10 Gene Sets"
