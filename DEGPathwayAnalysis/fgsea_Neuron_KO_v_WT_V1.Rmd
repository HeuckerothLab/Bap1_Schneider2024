...
subtitle: "Neuron DEGs KO vs. WT, across all clusters"
...
infile = "Neuron_GeneExpression_ConditionsCompared_V1.xlsx"
...
deg_tbl = read.xlsx(paste0(data_dir, infile), sheetIndex = 1) %>% # All clusters data is on Sheet 1
  # Fix col names
...
Data in table has been modified to show positive/negative values for KO (KO v. WT condition) by keep only rows where cluster == "KO", as the original file showed every gene for each condition separately.
...
fgsea_res_filt %>% dplyr::select(-prettyname) %>%
  # mutate(across(where(is.numeric), \(x) round(x, 6))) %>%
  datatable()

fwrite(fgsea_res_filt %>% dplyr::select(-prettyname), file = paste0(res_dir, "csvs/", analysis_name, "_", proj, "_", gsub(' ', '_', name), "_", version, ".csv"),
       sep = ",", sep2 = c("", "|", ""))
